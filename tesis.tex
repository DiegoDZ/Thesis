...
In this chapter we have introduced two important concepts that holds this dissertation. The first one is the quasi-equilibrium, and states that what we refer to equilibrium of a system depends of the time window we use. The second one, the separation of time scales,  is a consecuence of the first one.Markovian.
...
Finally, following the textbook of Grabert \cite{Grabert1982}, we have shown the general form of the dynamic equation of the relevant variables of a system, which has two terms, reversible and irreversible. The last one cointa the information about the past, is the term of memory. We have simplified it assuming the markovian aproximation. The memory-less equation obtained let us in the following chapter to obtain the dynamic equations of a CG variables of a system consisting on a liquid between two solid slabs.  
