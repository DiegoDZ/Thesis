...
The  purpose  of  the  present   paper  is  to  produce  the  discrete
...
the numerical  results.  Therefore, the Markovian  continuum theory of
...
In  the  present work  we  derive  with  the technique  of  projection
operators the  evolution equation for the  non-equilibrium averages of
the discrete  hydrodynamic variables.   The methodology  follows Refs. \Note{Faltan referencias}
...
consistently. We have  discussed in Refs. \Note{Me falta la referencia de arturo} \cite{DelaTorre2015,EspanolDonev2015} how  a method based
...
\begin{align}
  i{\cal L}\hat{\bf g}_\mu(z) &=\hat{\bf F}_\mu^{\rm s\to l}-\frac{\hat{\boldsymbol{\sigma}}_\mu-\hat{\boldsymbol{\sigma}}_{\mu-1}}{\Delta z}\esc{\bf n}
\label{Equivalence}
\end{align}
...
averages,  the ``spiky''  fields (\ref{rhogmumiccont})  with piece-wise
linear functions as
\begin{align}
...
tensors are, from  Eqs. (\ref{}) of Ref. \cite{CamargoBC2018}
\begin{align}
{\bf G}^{\alpha\beta z}
&=G^{||}{\bf T}^{\alpha\beta}+
