...
Esta tesis doctoral estudia el comportamiento nanoscópico de un fluido  cuando entra en contacto con un sólido. Derivaremos las ecuaciones de movimiento de un fluido en contacto con una esfera sólida de grandes dimensiones (de tal manera que la superficie de la esfera pueda ser aproximada a una pared) empleando la Teoría del \textit{Coarse-Graining} (ToCG). Para ello emplearemos la técnica de los operadores de proyección de Kawasaki-Gunton y asumiremos un comportamiento Markoviano para obtener un conjunto de ecuaciones sin complicados términos de memoria. 
Abordaremos el problema del \textit{plateau} que sufren la formulas de Green-Kubo presentes en los coeficientes de transporte de las ecuaciones de movimiento del fluido, ofrenciendo una expresión alternativa sin el problema del \textit{plateau}.  

Para validar la teoría y poder medir los coeficientes de transporte mediante simulaciones de dinámica molecular, derivaremos de nuevo las ecuaciones de la nanohidrodinámica a partir de unas variables relevantes discretas. 
Para ello dividiremos el sistema a lo largo del eje perpendicular a la superficie de la paredes en una serie de láminas de un grosor determinado separadas por planos nodales, de tal manera que las variables puedan ser definidas en dicha láminas o planos nodales.  
Con el fin de verificar que tal derivación es correcta, mostraremos que la ecuaciones obtenidas pueden ser interpretadas como una discretización de las ecuaciones continuas mediante el método del elemento finito de Petrov-Galerkin.  

La hipótesis de markovianidad es la única aproximación realizada para derivar las ecuaciones de la hidrodinámica. Para validarla emplearemos la teoría de Mori que nos da una expresión de la evolución temporal de las correleaciones de las variables relevantes del sistema. Concluiremos que la hipóteis Markoviana es válida siempre y cuando las correlaciones decaigan de forma exponencial. 
...
Tras establecer la metodología adecuada para tratar el problema, realizaremos simulaciones de dinámica molecular de un fluido confinado entre dos paredes sólidas. Descubriremos que para un grosor de la lámina de discretización del orden de la distancia molecular, la hipótesis de markovianidad no se cumple para los modos cercanos a la pared. Sin embargo, para láminas más gruesas, la hipótesis es validada a expensas de perder efectos de \textit{layering}. 

Mediremos la condición de contorno de \textit{slip} a partir de una definición microscópica de la longitud de \textit{slip} y de la posición hidrodinámica de la pared atómica. Comprobaremos que la expresión obtenida es la dada por Boquet y Barract \cite{Bocquet1994} y demostraremos que el coeficiente de fricción es una propiedad intrínseca de la superficie puesto que la distancia de \textit{slip} no depende del tamaño del canal.
...
\begin{figure}[]
  ...
  %\vspace{-5mm}
  \includegraphics[width=\linewidth]{SchemeBSlab-3}\vspace{-5mm}
  \caption[Scheme of the definition of the boundary slab of fluid]{Scheme of the definition of the boundary slab of fluid. The red line defines the function $U_{B}(z)$, for $B=1$ (left), $B=2$ (middle), and $B=3$ (right).\Note{Rehacerla. Mucho margen por arriba.}}
\label{Fig.Slab}
\end{figure}
