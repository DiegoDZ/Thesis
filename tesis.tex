...
We bin the system in slabs as is shown in Figure \ref{fig:WallsBox1}. The red lines are called {\it nodes} and they separated the {\it bins} in which the system is binned. We start from a discrete mass and momentum densities fields defined in the nodal planes in order to derive the hydrodynamic equations with the Kawasaki-Gunton projector. Also we validate the final discrete equations  with a discretization of the continumm hydrodynamic equations using the Petrov-Galerkin finite element discretization method.   
...
\begin{figure}
    \centering
    %\includegraphics[scale=0.3]{system_nodes_walls}
    \frame{\includegraphics[width=0.45\linewidth]{PRL3_gold2_wo_layers_wo_diffuse}}
    \frame{\includegraphics[width=0.45\linewidth]{PRL3_gold2_wo_diffuse}}
    \caption[Walls box]{A visual representation of the MD simulation with a sketch of the binning used. In red are depicted the nodal planes used.}
    \label{fig:WallsBox}
\end{figure}
...
\begin{figure}[]
  \centering
\includegraphics[scale=0.2]{SchemeBSlab-3}
\caption[Scheme of the definition of the boundary slab of fluid]{Scheme of the definition of the boundary slab of fluid. The red line defines the function $U_{B}(z)$, for $B=1$ (left), $B=2$ (middle), and $B=3$ (right).}
\label{Fig.Slab}
\end{figure}
...
plug  flow  considered   in  this  paper,  we   define  the  following
...
non-local viscosity  kernel $\eta_{B\nu}$  is not  zero for  $B=2$ and
...
\label{NSDisc}
\end{align}
This  equation  is to  be  compared  with the  non-local  hydrodynamic
...
discrete local hydrodynamic equation (\ref{NSDisc}) from the non-local
...
Fig. \ref{Fig-gxtPredictions} and Fig. \ref{Fig-gxt-vt}  the agreement  with the  non-local hydrodynamic
...
non-local viscosity  and non-local frictions.  The  friction forces in
...
decay of the plug flow in MD simulations we observe that the non-local
...
friction. The derivation  in the present paper, which is  based on the
...
from  the  non-local  equations.   This  is done  by  taking  a  local
approximation  for  the viscosity  kernel  in  the non-local  discrete
...
the non-local  theory with explicit  account of the  extended friction
...
bins  still display  non-local  effects.  For  example, the  non-local
...
As  mentioned above,  such non-local  effects are  needed in  order to
...
course,  for very  smooth  velocity fields,  the  non-locality of  the
...
This dissertation has three milestones:
\begin{enumerate}
  \item The derivation of the hydrodynamic equations (and its discretization) of a fluid in contact with a solid with the assumtion of the Markovian approximation. 
  \item The study of the Markovian behaviour for unconfined fluids as well as fluid between two solids slabs. 
  \item The measure of the slip lenght. \Note{Falta esto}
\end{enumerate}
In order to adress them we have
Chapter \ref{Chap:Theory}
%In order  to better  understand the problem of slip,  we have
proposed in  Chapter \ref{Chap:Theory} a continuum  hydrodynamic theory
derived  with  the  projection  operator  method  that  considers  the
interaction  of a  fluid with  a solid  object.  The  rationale is  to
construct a \textit{field theory} where the interaction with the solid
object  is  explicit in  such  a  way that  it  is  possible to  infer
\textit{a posteriori} the corresponding  boundary conditions.  This is
an entirely  new approach  to the  formulation of  boundary conditions
from  microscopic  principles  which differs  from  previous  attempts
discussed  above.  
%In  a series  of publications  \cite{2,3,4,5,6}, we
%have explored  different aspects  of the theory  and we  summarize our
%findings in what follows.

In   the    continuum   hydrodynamic   theory   presented    in   Chapter \ref{Chap:Theory}
%Ref. \cite{Camargo2018}  
...
are non-local in space and would  describe the layering of the density
...
valid.   One  can  reproduce  step  by  step  the  procedure  in  Ref.
...
non-local  viscosity  kernel  suffers dramatically  from  the  plateau
problem.  In Sec. of Chapter \ref{GKsi}, we have  shown that the plateau problem is
...
the explicit  forms of  the non-local  viscosity.  With  this measured
...
Non-Markovian effects  are detected from the  non-exponential decay of
...
After observe that thet suffer from the plateau problem, we have used the method presented in Chapter \ref{Chap:PBC}
for realiably extracting the transport kernels out of the plateau-problematic Green-Kubo formula. 
We have obtained excellent predictions with these kernels of the decaying of the average of the transverse momentum in a channel configuration when the initial velocity profile is a plug flow. 
We  have  infered the  slip  boundary
...
\Note{Quizá lo que me he dejado en las conclusiones del paper IV puede ir aquí.}
...
Hacer paredes más realistas. Explorar tamaños de bin. ¿Podemos capturar algo de layering?
...
\psfrag{gmu0t}{$g_{\mu=0}(t)$} 
\psfrag{t}{$t$} 
%  \includegraphics[scale=.6]{GreenFunction4.eps}
...
$\tilde{\nu}_\mu=\nu_0$.   We  plot   in  Figs.   \ref{GreenFunction3}
and \ref{GreenFunction4} this  evolution by suitably changing  the size
...
By using these results, Eq. (\ref{Layer}) becomes
\begin{align}
{\bf F}^x_B=
& S\sum_{\nu=1}^{N_{\rm bin}} {\cal V}_\nu 
\eta_{B\nu}\frac{\overline{\bf v}^x_{\nu+1}-\overline{\bf v}^x_{\nu}}{\Delta z}
