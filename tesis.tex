...
\label{ThermalWave}
\end{align}
Together with equation (\ref{rgo}) and equation (\ref{oh}) \Note{Primera expresión:campo de densidad igual a masa}
\begin{align}
...
covariances  (see   equation.  (\ref{cg1})-(\ref{covariances}))   and  this
...
\label{Fd2}
\end{align}
We can obtain an expression for the  hydrodynamic functional (\ref{oleg}) replacing the terms $\Phi[\lambda]$, $\boldsymbol{\lambda}_g({\bf r})$, $\boldsymbol{\lambda}_P$, and $\lambda_R\cdot{\bf R}$ by the equation. (\ref{xpos}), $(\ref{nu})$, and $(\ref{leg})$, respectively. 
\begin{align}
  {\cal H}[\rho,{\bf g},{\bf R},{\bf P}] =& 
  -\int d{\bf r} \rho({\bf r})\lambda_{\rho}({\bf r}) 
...
In this subsection we consider the reversible part for the case that the CG variables are those in equation. (\ref{CGvar}).
...
equation  (\ref{chempot}). \Pendiente{Under the action of the Galilean operator, the relevant ensemble is Gaussian in momentum and zero centered.}  
...
computed  in   equation  (\ref{fllr})  of  the   appendix.   By  collecting
...
\end{align}
where we have used equation (\ref{Fls}) in the Appendix.
...
manifestly  Galilean   invariant.   By  using  equation.   (\ref{gls})  and
...
equation.   (\ref{finaleq}), (\ref{GK}),   (\ref{Irreversibleforces}), 
...
equation.  (\ref{finaleq}).  The  equation for  the evolution  of the  mass
...
$\boldsymbol{\nabla}\esc\boldsymbol{\Sigma}$  in equation.  (\ref{finaleq})
...
As a consequence, the equilibrium state of equation. (\ref{finaleq}) is the
...
\label{eq1}
\end{align}
From equation.   (\ref{Fd2}), the first equation  (\ref{eq1}) simply states
...
boundary conditions to be specified for equation.  (\ref{finaleq})?  In the
...
equation.  (\ref{finaleq}).  Finally, in  the third step the discretization
...
The derivatives of the entropy are  given in equation.  (\ref{derS}) in the
...
tensors are, from  equation. \Pendiente{(104) and (105)} of Ref. \cite{CamargoBC2018}
\begin{align}
{\bf G}^{\alpha\beta z}
&=G^{||}{\bf T}^{\alpha\beta}+
...
tensors  in equation  (\ref{etasimple})-(\ref{gg1})  with ${\bf  n}$ or  the
...
inserting the equation. (\ref{FluidStress}) and (\ref{Irreversibleforces}) into (\ref{finaleq}) and then multiply with
...
$\rho_\mu,{\bf v}_\mu$ in equation. (\ref{fulldeltapsi}) is the term
...
fluid can be set equal to the corresponding homogeneous transport coefficientes calculated at a locally averaged density in a way similar to what  is done in equationilibrium Density  Functional Theory for the formulation of Weighted Density  Approximation models for the free energy functional \cite{Tarazona2008}.  %equation7.27
...
%similar to what  is done in equationilibrium Density  Functional Theory for
...
dynamic is Markovian.  equation  (\ref{GKCorrected})  is the main result of
the present paper.  equation   (\ref{GKCorrected}) is conceptually pleasing
...
\begin{align}
    \frac{d}{dt}C(t) = -(L+M^*)\cdot C^{-1}(0)\cdot C(t),
\end{align}
where we have use equation. (\ref{AproxC}),  (\ref{Lambda}) and the normalized correlation matrix defined as
\begin{align}
  c(t)=C^{-1}(0)\esc C(t)
\end{align}
Note that the reversible matrix $L$ vanishes because it involves an equilibrium average of a function that is even in the momentum variables. Therefore
...
\label{E1}
\end{align}
Therefore,  equation.  (\ref{MmunuMat}), (\ref{E1})  lead  to  the  following
...
equation  (\ref{FouFin}) we will infer the non-local shear viscosity matrix
...
\caption[Diagonal elements  $\tilde{\Lambda}_{\mu\mu}(t)$ of  the Fourier transform of $\Lambda(t)$]{The  diagonal elements  $\tilde{\Lambda}_{\mu\mu}(t)$ of  the
  ...
  intervals of $0.02$.  The dotted line is the local approximation equation
...
$\eta_0(t)$ which,  according to equation.  (\ref{eta00})  and (\ref{etat})
...
have shown in  equation (\ref{FouFin}) how to evaluate  the nonlocal shear
...
\label{FouFint}
\end{align}
which, according  to equation (\ref{FouFin})  should have a plateau  if the
...
The kinematic viscosity  in Fourier space is  obtained by transforming equation (\ref{numunu0})
...
% \end{align}
% }
equation.  (\ref{dCMt}) and (\ref{MMatrix1}) lead to the following mathematical identity
\begin{align}
...
\begin{align}
  c(t)=C^{-1}(0)\esc C(t) 
\end{align}
which equals  the identity  matrix at  $t=0$.  In  equation  (\ref{Mstar}),
