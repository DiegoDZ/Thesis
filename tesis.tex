...
observation that the velocity of fluids  may be non-zero near the wall
...
In the first chapter we review Non-Equilibrium Statistical Mechanichs (NESM) in order to state the foundations in which the following chapters are based. 
We introduce two projection operator that allows us to obtain hydrodynamic equations: the Kawasaki-Gunton projector \cite{Kawasaki1973} and the Mori projector \cite{Mori1965}.  The first one allows one to obtain  non-linear closed equations
...
The resulting equations contain transport coefficients given by  Green-Kubo formulae, which suffer from the plateau problem when the separation of time scales is not large.  
...
and  it involves the so called projected
...
dynamics in terms of the  time integral of correlation functions
  of the  time derivatives of  the CG variables.  These  formulae for
transport   coefficients  are   the  celebrated   Green-Kubo  formulae
...
\label{Sec:Mori}
\Note{referencias y no duplicar información de la sección anterior.}
In this section we present  a summary of Mori's theory \cite{Mori1965}
in order to set the notation. At a microscopic level of description we
assume that  the system is  well described by classical  mechanics and
...
system.  Hamilton's equations can be written in a very compact form as
...
describe  the evolution  of  correlations and  averages towards  their
equilibrium values.
...
scale.   On the  other hand,  it  is assumed  that the  time scale  of
...
that  at $t=0$  becomes  the identity  matrix.   The  Markovian
  ...
  such a  separation of  time scales as  some parameter  becomes small
...
separation  of  time scales  in  order  to approximate  the  projected
...
Hamiltonian that render the evolution  non-ergodic with respect to the
...
ambiguous values for the transport coefficients.  

\section{Summary}
\Note{Falta actualizarlo}
...
%The Markovian approximation assumes that the there CG variables evolve in a time scale much larger than the memory time scale. This implies that there exists  a time-independent matrix $M^*$
%such that  the linear integro-differential term  in Eq. (\ref{exactC})
%can  be  approximated  by  a  memory-less term,  also  linear  in  the
%correlation matrix
%\begin{align}
...
\label{Markov1}
\end{align}
% Under  the  Markovian  approximation  Eq.  (\ref{Markap}),  the  exact
% equation (\ref{exact}) becomes
% \begin{eqnarray}
% \frac{d}{dt}{\hat{A} }(t) = -LC^{-1}(0) \hat{A}(t)-M C^{-1}(0)\hat{A}(t) +F^+(t)
% \label{MSDE}
% \end{eqnarray}
% which is,  in principle, an ordinary  differential equation.  However,
% due to the  rapidly varying correlation of $F^+(t)$  this equation can
% be interpreted as an stochastic differential equation for times larger
% than the correlation of $F^+(t)$.
% In  the  Markovian approximation,  if  we  multiply (\ref{MSDE})  with
% $\hat{A}(0)$ and average over the equilibrium ensemble we obtain
...
%\Del{ \section{The standard Green-Kubo formula is problematic}
%}
\label{Sec:GKno}
The  usual  rationale  for   justifying  the  Markovian  approximation
(\ref{Markov1})  goes  as  follows  \cite{Grabert1982,Kubo1991}.   The
...
exists, \textit{provided} the dynamics can still be approximated
  by a  Markovian equation is  Markovian in  such a  way that
...
% {\color{magenta} From the two exact Eqs (\ref{exactC}) and (\ref{Math1}) we then
% obtain
% \begin{align}
% M(t)&=\int_0^tdt' \Gamma(t-t')\esc c(t')
% \label{exactMC}
% \end{align}
% If the Markov approximation (\ref{M+c}) is valid we obtain 
% \begin{align}
% M(t)&\simeq M^+(t)\esc c(t)
% \label{exactMC2}
% \end{align}
% and if $M^+(t)\to M^*$ after a time $\tau$, we end up with
% \begin{align}
% M(t)&\simeq M^*\esc c(t)
% \label{exactMC3}
% \end{align}
...
of the CG variables.
Although   the   time   integral   in    the   left   hand   side   of
(\ref{GKCorrected0}) has  no plateau
...
integral  (\ref{GK0}) has  no plateau  -- in  fact, it  decays as  the
...
(\ref{GK0}) for the transport coefficients.
...
reduced  towards   the  micro  and   nanoscale,  a  large   number  of
...
We show  that  the  usual Green-Kubo  formulae  suffer
...
kernels.  The  present formulation with corrected  Green-Kubo formulae
...
in non-equilibrium  MD simulations  that the  plug flow  considered in
...
\caption{The slip length $\delta(t)$ as a function of time for the two
  ...
  friction  forces, leading  to  a constant  non-zero  slip length  of
