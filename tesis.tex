...
surface,  showing  that  the   no-slip  boundary  condition  does  not
necessarily holds at the nanoscale.  Slip at nanoscales has received a
large amount of attention from the  simulation point of view since the
pioneering        work       of        Thomson       and        Robins
\cite{Thompson1990,Koplik1995,Troian1997}.    Statistical  Mechanics (SM)
approaches to infer  the slip length from  Green-Kubo expressions have
been
undertaken\cite{Bocquet1994,Barrat2003,Petravic2007,Hansen2011,Kannam2011,Bocquet2013,Huang2014,Ramos-Alvarado2016,Daivis2018},
with  an  interesting  debate  that has  not  been
concluded yet \cite{Ramos-Alvarado2016}.
...
vanishes  at the  surface of  fixed solid  walls \cite{Batchelor1967}.
...
have a plateau  and it decays to zero for  large $\tau$.  However, the
...
\cite{Barrat1999,Bocquet2007a,Kobryn2008,Kobryn2011}.
...
% {\blue However, linear response theory is  of limited value as it does
%   not produce a field theory, it only allows one to identify transport
%   coefficients.     As   discussed    in   Petravic    and   Harrowell
%   \cite{Petravic2007}, linear response theory when used with arbitrary
%   perturbations  may   lead  to   erroneous  results   if  incorrectly
%   interpreted.}
...
% Hynes, et al.  \cite{Hynes1979} also discused how the
% impenetrability  condition needs  to  be modified  when the  spherical
% particle becomes of  molecular size. 
...
the solid-fluid friction coefficient which,  as shown in Ref. \cite{2}
...
\subsection{Evolution of the flow in NEMD simulations}

\begin{figure}[!h]
\includegraphics[width=\linewidth]{gxtPredictions-17nodes-WALLS}
\caption{Top  panel: the  momentum $g(t)$  profile at  different times
  ...
  Also  plot  is  the  prediction  given  by  (\ref{gsol}),  which  is
  undistinguishable  from the  measured  profile.   Left panel:  the
  momentum $g_\mu(t)$  for the nodes $\mu=1,2,4$  (in ascending order)
  as  a  function  of  time  (blue),  and  the  predictions  given  by
  (\ref{gsol}) (orange).}
\label{Fig.gxevol}
\end{figure}
...
\begin{figure}[!h]
    \centering
\includegraphics[scale=0.41]{vtCorrected-17nodes-WALLS}
\caption{  The velocity  profile  obtained from  the momentum  through
  (\ref{ov}).  \Note{Decir arruguitas}}
\label{Fig.vevol}
\end{figure}
...
\caption{%Check of Eq. (\ref{FBv}) for  $B=1$ (top), $B=2$ (middle) and  $B=3$ (bottom). 
Check of Eq. (\ref{FBv}) for $B=2$. Green is the measured force $F_B(t)={\bf F}^x_B(t)$,
  blue  is  the  right  hand  side of  (\ref{FBv}),  and  red  is  the
  difference between both quantities, and  a reflection of the failure
  of  the  Markovian  equation   (\ref{InFullLambda})  at  very  short
  times. The vertical line is at the plateau time $\tau=0.3$. Beyond
  this  time the  Markovian equation  is valid  but the  force is  not
  zero. For times larger than approximately $t=2$, the force vanishes.}
\label{Fig.FBcheck}
\end{figure}
...
\caption{The total friction $\gamma(\tau)$ as  a function of the upper
  ...
  (\ref{FinMic}).  The right panel is in  logscale.  The  vertical line  is  at
