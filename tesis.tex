...
The form of the projected currents is computed explicitly in the appendix.
% \begin{widetext} 
%   {\blue The relevant ensemble is  a generalized canonical ensemble of
%     exponential form.  We consider now  the situation in which  we may
%     approximate the  generalized canonical  ensemble by  a generalized
%     microcanonical  ensemble, where  the position  and momenta  of the
%     solid sphere is fixed.

% The average with the  relevant ensemble of an arbitrary phase
%     function $\hat{B}(z)$ is
% \begin{align}
%   {\rm Tr}[\overline{\rho}_t \hat{B}]
% &=\frac{1}{\Xi[\lambda_l,\lambda_s]}
%  \sum_{N=0}^\infty \frac{1}{N!h^{3N}}
% \int dqdp dq'dp'
% \exp\left\{-\beta H_N-\beta \sum_{i=1}^Nm\lambda_\rho({\bf
%     r}_i)-\beta \sum_{i=1}^N{\bf p}_i\esc\boldsymbol{\lambda}_g({\bf q}_i)\right\}
% \nonumber\\
% &\times\exp\left\{-\beta \boldsymbol{\lambda}_{R}\esc\hat{\bf R}(z)
% -\beta \boldsymbol{\lambda}_{P}\esc\hat{\bf P}(z)\right\}\hat{B}(z)
% \nonumber\\
% &=\int d{\bf R} d{\bf P}
% \exp\left\{-\beta \boldsymbol{\lambda}_{R}\esc{\bf R}
% -\beta \boldsymbol{\lambda}_{P}\esc{\bf P}\right\}
% \frac{1}{\Xi[\lambda_l,\lambda_s]}
%  \sum_{N=0}^\infty \frac{1}{N!h^{3N}}
% \int dqdp dq'dp'\nonumber\\
% &\times
% \exp\left\{-\beta H_N-\beta \sum_{i=1}^Nm\lambda_\rho({\bf
%     r}_i)-\beta \sum_{i=1}^N{\bf p}_i\esc\boldsymbol{\lambda}_g({\bf q}_i)\right\}
% \delta({\bf R}-\hat{\bf R}(z))\delta({\bf P}-\hat{\bf P}(z))\hat{B}(z)
% \end{align}
% where  we  have  introduced  the identity  $\int  d{\bf  R}\delta({\bf
%   R}-\hat{\bf    R}(z))=1$,     etc.     and    we     have    defined
% $\Xi[\lambda_l,\lambda_s]$,                                       with
% $\lambda_l=\lambda_\rho,\boldsymbol{\lambda}_g$                    and
% $\lambda_s=\boldsymbol{\lambda}_{R},\boldsymbol{\lambda}_P$.
% \begin{align}
%   {\rm Tr}[\overline{\rho}_t \hat{B}]
% &=\int d{\bf R} d{\bf P}
% \exp\left\{-\beta \boldsymbol{\lambda}_{R}\esc{\bf R}
% -\beta \boldsymbol{\lambda}_{P}\esc{\bf P}\right\}
% \frac{\Xi[\lambda_l,0]}{\Xi[\lambda_l,\lambda_s]}P_{a}({\bf R},{\bf P})
% \llangle \hat{B}\rrangle^{\rho {\bf g}}_{{\rm mic} \;{\bf R}{\bf P}}
% \label{gencanmic}
% \end{align}
% where we have defined
% \begin{align}
%   \llangle \hat{B}\rrangle^{\rho {\bf g}}_{{\rm mic} \;{\bf R}{\bf P}}&\equiv
% \frac{1}{\Xi[\lambda_l,0]P_{a}({\bf R},{\bf P})}
%  \sum_{N=0}^\infty \frac{1}{N!h^{3N}}
% \int dqdp dq'dp'\nonumber\\
% &\times
% \exp\left\{-\beta H_N-\beta \sum_{i=1}^Nm\lambda_\rho({\bf
...
% \delta({\bf R}-\hat{\bf R}(z))\delta({\bf P}-\hat{\bf P}(z))\hat{B}(z)
% \nonumber\\
% P_{a}({\bf R},{\bf P})&\equiv
% \frac{1}{\Xi[\lambda_l,0]} \sum_{N=0}^\infty \frac{1}{N!h^{3N}}
% \int dqdp dq'dp'\nonumber\\
% &\times
% \exp\left\{-\beta H_N-\beta \sum_{i=1}^Nm\lambda_\rho({\bf
%     r}_i)-\beta \sum_{i=1}^N{\bf p}_i\esc\boldsymbol{\lambda}_g({\bf q}_i)\right\}
% \delta({\bf R}-\hat{\bf R}(z))\delta({\bf P}-\hat{\bf P}(z))
% \nonumber\\
% \Xi[\lambda_l,0]&\equiv \sum_{N=0}^\infty \frac{1}{N!h^{3N}}
% \int dqdp dq'dp'
% \exp\left\{-\beta H_N-\beta \sum_{i=1}^Nm\lambda_\rho({\bf
%     r}_i)-\beta \sum_{i=1}^N{\bf p}_i\esc\boldsymbol{\lambda}_g({\bf q}_i)\right\}
% \end{align}
% Here $P_{a}({\bf R},{\bf  P})$ is the normalized  probability that the
% solid sphere  has center of  mass position and momentum  ${\bf R},{\bf
%   P}$  when the  microscopic ensemble  is the  relevant ensemble  with
% $\boldsymbol{\lambda}_R=0,\boldsymbol{\lambda}_P=0$.  This probability
% depends on  the full set  $a$ of  averages of the  relevant variables,
% because the  conjugate variables $\lambda_\rho,\boldsymbol{\lambda}_g$
% depend on this full set.  The average $ \llangle \hat{B}\rrangle^{\rho
%   {\bf g}}_{{\rm mic} \;{\bf R}{\bf  P}}$ is canonical with respect to
% $\rho,{\bf  g}$  but  microcanonical  with respect  to  ${\bf  R},{\bf
%   P}$.  Eq.  (\ref{gencanmic})  relates   the  fully  canonical  average  ${\rm
%   Tr}[\overline{\rho}\hat{B}]$   with  the   partially  microcanonical
% average  $\llangle \hat{B}\rrangle^{\rho  {\bf  g}}_{{\rm mic}  \;{\bf
%     R}{\bf P}}$ and can be written as

% \begin{align}
%   {\rm Tr}[\overline{\rho}_t \hat{B}]&=
% \int d{\bf R} d{\bf P}G({\bf R},{\bf P})\llangle \hat{B}\rrangle^{\rho {\bf g}}_{{\rm mic} \;{\bf R}{\bf P}}
% \end{align}
% with the definition 
% \begin{align}
%   G({\bf R},{\bf P})&\equiv
...
% \frac{\Xi[\lambda_l,0]}{\Xi[\lambda_l,\lambda_s]}P_{a}({\bf R},{\bf P})
% \end{align}
% This function can be interpreted as a probability distribution, as it is normalized to unity
% \begin{align}
% \int d{\bf R}d{\bf P}    G({\bf R},{\bf P})&=1
% \end{align}
% The maximum ${\bf R}^*,{\bf P}^*$ of this probability occurs when
% \begin{align}
% \boldsymbol{\lambda}_{R}&=k_BT\frac{\partial}{\partial{\bf R}}
% \ln P_{a}({\bf R}^*,{\bf P}^*)
% \nonumber\\
% \boldsymbol{\lambda}_{P}&=k_BT\frac{\partial}{\partial{\bf P}}
% \ln P_{a}({\bf R}^*,{\bf P}^*)
% \end{align}
% I  would   like  to  find   that  the  solution  is   ${\bf  R}^*={\rm
%   Tr}[\overline{\rho}_t\hat{\bf     R}]$    and     ${\bf    P}^*={\rm
%   Tr}[\overline{\rho}_t\hat{\bf         P}]$.          


% {\bf The question I want to answer is: How to compute operationally the relevant
% ensemble averages?}

% }\end{widetext}
% {\red \subsection{Onsager relations} Onsager  relations arise from the
%   property of microscopic reversibility  and its effect on equilibrium
%   correlation functions.   We briefly  review the  argument.  Consider
%   the  equilibrium   correlation  function  of  two   phase  functions
%   $\hat{A}(z),\hat{B}(z)$, i.e.
% \begin{align}
%   \langle \hat{A}(t)B\rangle^{\rm eq}&=\int dz \rho^{\rm eq}(z)\hat{A}(T_tz)\hat{B}(z)
% \end{align}
% where  $T_tz$ is  the  solution of  Hamilton's  equation with  initial
% condition $z$. A change of  variables $z'=T_tz$ has unit jacobian and,
% because  the  equilibrium  ensemble  depends  only  on  the  dynamical
% invariants, we have $\rho^{\rm  eq}(T_tz)=\rho^{\rm eq}(z)$ leading to
% the stationarity condition.
% \begin{align}
%   \langle \hat{A}(t)B\rangle^{\rm eq}&=\int dz \rho^{\rm eq}(z)\hat{A}(T_tz)\hat{B}(z)
% \nonumber\\
% &=\int dz' \rho^{\rm eq}(z')\hat{A}(z')\hat{B}(T_{-t}z')=  \langle A\hat{B}(-t)\rangle^{\rm eq}
% \end{align}
% On  the other  hand,  microscopic  reversibility implies  $T_t\epsilon
% z=\epsilon T_{-t}z$  where $\epsilon$  is a  matrix that  reverses the
% sign of  the momenta in  the microstate  $z$. The change  of variables
% $\epsilon z'=z$ now gives
% \begin{align}
%   \langle \hat{A}(t)B\rangle^{\rm eq}&=\int dz' \rho^{\rm eq}(\epsilon z')\hat{A}(T_t\epsilon z')\hat{B}(\epsilon z')
% \nonumber\\
% & =
% \int dz' \rho^{\rm eq}(z')\hat{A}(\epsilon T_{-t}z')\hat{B}(\epsilon z')
% \end{align}
% If both phase functions transform in the same way under time reversal,
% i.e. $\hat{A}(\epsilon z)=\pm \hat{A}(z)$ $\hat{B}(\epsilon z)=\pm \hat{B}(z)$ this implies
% \begin{align}
%   \langle \hat{A}(t)B\rangle^{\rm eq}&=  \langle \hat{A}(-t)B\rangle^{\rm eq}
% \end{align}
...
\end{align}
% Once we assume that the correlations are given in terms of equilibrium
% averages (conditional to  have fixed the sphere), we expect  to have a
% number  of  time and  space  invariance  symmetries in  the  transport
% coefficients  that  derive from  the  above  symetry.}  
...
taken  in Sec.  \ref{Sec:Trans}  for the  present  case.  The  most
...
\begin{align}
  \hat{\bf F}^{\rm s\to l}_{\bf r}&=\sum^{NN'}_{ii'}\hat{\bf F}_{ii'}\delta({\bf r}-{\bf q}_i)
\nonumber\\
&=\sum^{NN'}_{ii'}{\bf F}({\bf r}-{\bf q}_{i'})\delta({\bf r}-{\bf q}_i)
\nonumber\\
...
the  transport kernels  involving  the projected  force vanish.   This
