...
\section{Introduction}
\Note{Este capítulo lo reduciría para que la tesis no ocupe tantas páginas. Simplemente comentaría que tenemos un artículo en el que se discretizan las ecuaciones obtenidas en el capítulo 1. Pondría el método de discretización y la ecuaciones resultantes discretizadas. De todos modos, aquí presento un capítulo en el que está todo lo que aparece en el paper excepto la sección en la que se obtienen las mismas ecuaciones discretizadas con el método de Petrov-Galerkin.}
...
\begin{align}
  i{\cal L}\hat{\bf g}_\mu(z) &=\hat{\bf F}_\mu^{\rm s\to l}(z)-\frac{\hat{\boldsymbol{\sigma}}_\mu(z)-\hat{\boldsymbol{\sigma}}_{\mu-1}(z)}{\Delta z}\esc{\bf n}
\label{Equivalence}
\end{align}
...
(\ref{ExactRev}) explicit in the  CG variables.  This approximation is
named  \textit{linear   for  spiky   approximation}  (LFSA)   in  Ref.
\cite{Donev} and  consists on  approximating within  ensemble averages
the  ``spiky''  fields  (\ref{rhogmumiccont}) with  piece-wise  linear
functions as
\begin{align}
  \hat{\rho}_{\bf r}(z)&\simeq \hat{\rho}_{\sigma}(z)\psi_\sigma({\bf r})
\nonumber\\
  \hat{\bf g}_{\bf r}(z)&\simeq \hat{\bf g}_{\sigma}(z)\psi_\sigma({\bf r})
\label{LFSA}
\end{align}
The left  hand side contains  sums of coarse  delta functions,
while the  right hand  side are piece-wise  linear functions  given in
terms   of   the  discrete   values   $\hat{\rho}_{\sigma}(z), \hat{\bf
  g}_\sigma(z)$.   This  approximation  is  expected  to  hold  inside
ensemble averages and to be a good one if there are many particles per
bin. \Note{Pensar un poco mejor por qué} 

In the LFSA, the average of the density field becomes explicit
in the discrete density variables. 
\begin{align}
  \llangle \hat{\rho}_{\bf r}\rrangle^{\lambda{\bf v}}&\simeq\rho_\mu\psi_\mu({\bf r})=\overline{\rho}({\bf r})
\end{align}
and the density matrix (\ref{rhomunu}) becomes explicit in the discrete density 
\begin{align}
  \rho_{\mu\nu} \simeq \llg\delta_\mu\psi_\nu\psi_\sigma\rlg\rho_\sigma
=\llg\delta_\mu\psi_\nu\overline{\rho}\rlg
\label{calR1}
\end{align}
...
\begin{align}
\llangle i{\cal L}  \hat{\rho}_{\mu}\rrangle^\lambda&= 
\llg \overline{\rho}\;\overline{\bf v}\esc\boldsymbol{\nabla}\delta_\mu\rlg
\nonumber\\
\llangle i{\cal L}  \hat{\bf g}_{\mu}\rrangle^\lambda &=
...
functions of the discrete CG variables $\rho_\mu,{\bf v}_\mu$.
...
\end{align}
where $\llangle\cdots\rrangle^\lambda$  is an  average taken  with the
relevant  ensemble.  Therefore,  the dissipative  matrix  depends,  in
...
\begin{align}
  i{\cal L}\hat{\rho}_\mu(z)&=\int d{\bf r}\hat{\bf g}_{\bf r}(z)\esc\boldsymbol{\nabla}\delta_\mu({\bf r})
\nonumber\\
&\simeq \hat{\bf g}_\nu(z)\llg\psi_\nu\boldsymbol{\nabla}\delta_\mu\rlg
\end{align}
...
projected  current vanishes,  ${\cal Q}i{\cal  L}\hat{\rho}_\mu(z)=0$.
...
\begin{align}
\tilde{\lambda}_\mu&={M^\Phi}^{-1}_{\mu\nu}{\cal V}_{\nu}\lambda_{\nu}
\nonumber\\
\tilde{\bf v}_\mu&={M^\Phi}^{-1}_{\mu\nu}{\cal V}_{\nu}{\bf v}_{\nu}
\label{overlined}
\end{align}
...
\nonumber\\
&+\sum_{\nu=1}^{N_{\rm bin}}{\cal V}_\nu\frac{\left[{\bf G}_{\mu\nu}-{\bf G}_{\mu\nu-1}\right]}{\Delta z}:{\bf n}\tilde{\bf v}_\nu
\nonumber\\
...
tangential  ${\bf T}=\boldsymbol{\delta}-{\bf  n}{\bf n}$  projectors.
We note that the tangential projector satisfies
\begin{align}
{\bf  T}^{\alpha  z}=\delta^{\alpha z}-{\bf n}^\alpha{\bf n}^3=0\quad\quad \forall \alpha
\label{T3}
\end{align}
and, therefore, the required components of the viscosity tensor for an
isotropic  wall are  given,  from  Eq (\ref{viscosityTangential}) in the Appendix
\begin{align}
  \boldsymbol{\eta}^{\alpha z\gamma z}&=  
\eta^{\bot} {\bf n}^\alpha  {\bf n}^\gamma  +\eta^{||} {\bf T}^{\alpha\gamma}
\end{align}
\Pendiente{The  required components  of the  third order
tensors are, from  Eqs (??) of Ref. \cite{CamargoBC2018}}
\begin{align}
{\bf G}^{\alpha\beta z}
&=G^{||}{\bf T}^{\alpha\beta}+
...
When we assume rotational invariance, it makes sense to look separately to the 
...
(\ref{summary_munu})  are  one  of  the main  result  of  this chapter.
They  display the  discrete hydrodynamics  of a  fluid confined
by  parallel  walls  and  moving  with a  flow  that  is  translation
invariant along the walls.

Note that  one prediction  of these equations  is that  the tangential
velocity  components are  uncoupled from  the dynamics  of the  normal
component and the density. Also,  the tangent components do not depend
on the  free energy of  the system.  This means that  shearing motions
do not affect the structure of the density field.
...
%inserting (\ref{FluidStress}) and (\ref{Irreversibleforces}) into (\ref{finaleq}) and then multiply with
...
%\label{interpolated2}
%\end{align}
%where  we  have  used  (\ref{PsiFE})   and  the  the  discrete  fields
...
In Ref. \cite{CamargoDiscrete2018} we show that if we discretize the continuum equations (\ref{finaleq}) 
with a Petrov-Galerkin finite element discretization method, and assume that the 
transport kernels are isotropic under rotations around the axis perpendicular to 
the slabs, we recover the discrete equations \ref{TheoryDiscFin} that have been 
obtained directly from the projection operator technique that starts directly from the discrete variables (\ref{CGvar}). This consistency property is very pleasant and is a consequence of the particular way of defining the CG variables (\ref{CGvar}), in terms of finite element basis functions.
...
\begin{eqnarray}
  \overline{\rho}(z)&=&\frac{1}{\Xi(\lambda,{\bf v})}\exp\left\{-\beta \hat{H}_N(z)
-\beta\left(M^\psi_{\mu\nu}\lambda_\nu \hat{\rho}_{\mu}(z)
...
The resulting equations of motion have, with this definition, a direct counterpart with a discretization equations of the continuum hydrodynamic theory as presented in Sec. \ref{Sec:DiscreteBasis}.
...
\int dzdz'
\exp\left\{-\beta H_N(z)-\beta\left( M^\psi_{\mu\nu}\lambda_\nu\hat{\rho}_{\mu}(z)
-M^\psi_{\mu\nu}{\bf v}_\nu\esc\hat{\bf g}_{\mu}(z)\right)\right\}
\label{xiln0}
\end{align}
...
- \llg\delta_\mu\llangle\hat{\rho}\rrangle^{\lambda{\bf v}}
