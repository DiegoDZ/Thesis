...
We have introduce the kinematic viscosity as give by $\nu=\eta_{\mu}(\tau)/\rho_{\mu}(\tau)$ that takes the value $\nu=1.41$. Even though $\eta_{\mu}(\tau)$ and $\rho_{\mu}(\tau)$ depend on $\tau$,$\mu$, the kinematic visocisty is constant and does not depend on $\tau$ and $\mu$. \Note{No sé si poner la gráfica. Es muy fea.}
We see that 
...
\caption{Comparison  of   the  error   between  local   and  non-local
  ...
  Fig. \ref{Fig:LocalPrediction} and Fig. \ref{Fig:LocalPrediction2}.  
