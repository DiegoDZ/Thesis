...
\begin{frame}{Motivation}
  \begin{itemize}
    \item<1-> Great interest in the study of fluids in contact with solids in the nanoscale ($1-100$ nm).
    \item<2-> \alert{Density layering} $\rightarrow$ DFT for equilibrium situations $\leadsto$ DDFT for the study of the dynamic behaviour of the fluid. 
    \item<3-> \alert{Slip boundary condition}
          \begin{align}
...
\end{align}
      ...
      \item<6-> The expression for $\gamma$ suffers from the \alert{plateau problem}. 
    \end{itemize}
\end{frame}
...
\begin{frame}{Simpler theory}
  \begin{itemize}
    ...
    \item<2-> Two simplifications
      \begin{enumerate}
        \item The interactions felt by the fluid due to the walls are statistically planar and isotropic (\alert{planar walls}).
        \item \alert{Planar flows}: the hydrodynamic flow depends only on the coordinate perpendicular to the walls. 
      \end{enumerate}
    \item<3-> Under the simplification we may separate the evolution of the selected variables in two contributions: normal and tangent.
...
%\end{itemize}
    \begin{itemize}
     ...
     \item $28749$ particles.
     \item LJ potential truncated at $\sigma=2.5$.
     \item $dt=2\cdot 10^{-4}$ in reduced units. 
    \end{itemize}
  \end{center}
\end{frame}
...
\begin{frame}{Confined fluid. Simulation set up}
\begin{figure}
    \centering
...
\end{figure}
    \begin{itemize}
     ...
     \item $28175$ particles.
     \item LJ potential truncated at $\sigma=2.5$.
     \item $dt=2\cdot 10^{-3}$ in reduced units. 
    \end{itemize}
\end{frame}
   ...
   \begin{itemize}
     ...
     \item<1-> \alert{Equilibration} stage
       ...
       \begin{itemize}
         ...
         \item $z$ axis binned in $66$ bins $\mu$ ({$\boldsymbol{\Delta} {\bf z=0.5}\boldsymbol{\sigma}$}).
         \end{itemize}
       \item<3->[]
         \begin{figure}
...
\end{figure}
\item<2-> Evolution of different eigenvalues $\tilde{C}_{\mu\nu}(t)$
\begin{figure}[h!]
...
%    The eigenvalues $\tilde{C}_{\mu}(t)$ of the correlation matrix $C(t)$ for $\mu=59,60$ which are identical and superimpose (left) and the corresponding eigenvectors $u_{\mu}$ in blue and orange, respectively (right).
  Eigenvalues $\tilde{C}_{\mu}(t)$ of $C(t)$ for $\mu=59,60$ and the corresponding eigenvectors $u_{\mu}$ in blue and orange, respectively.
  \begin{figure}[h!]
...
\end{figure}
\item<2-> We need to add a new layer of solid atoms
\begin{figure}[h!]
\includegraphics[width=0.9\linewidth]{bin_size-top-bottom_17nodos_cut}
\end{figure}
  \end{itemize}
\end{frame}
...
\end{figure}
  \begin{itemize}
    ...
    \item Only after the time in which the transport matrix reached a plateau, and hence a Markovian theory holds, it is expected that we get correct predictions. 
  \end{itemize}
\end{frame}
