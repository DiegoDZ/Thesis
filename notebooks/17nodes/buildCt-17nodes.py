...
#Inputs
#----------------------------------
nSteps, nCols = 7500, 49                                 #nCols = nNodesBlock ** 2
dt            = 0.004
...
if gxgx=='y':
    ...
    block1 = np.loadtxt('corr-gxgx-WALLS-block1-17nodes-AVG.dat')
    block2 = np.loadtxt('corr-gxgx-WALLS-block2-17nodes-AVG.dat')
    corr_gxgx_neigh_from8to11 = np.loadtxt('Ct-canalNeigh-8-11-WALLS-17nodes.dat')
    ...
    np.savetxt('Ct-WALLS-17nodes.dat'          , C)
    #np.savetxt('C-t0.0-WALLS.dat'          , reshape_vm(C[step(0),:]))
    #np.savetxt('C-t0.25-WALLS.dat'         , reshape_vm(C[step(0.25),:]))
    #np.savetxt('C-t0.5-WALLS.dat'          , reshape_vm(C[step(0.5),:]))
...
#---------------Build matrices of correlations---------------
    ...
    np.savetxt('SxzSxz-WALLS-17nodes.dat'         , C)
