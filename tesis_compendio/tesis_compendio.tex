...
This sentence was written at the end of the 18th century by Pierre-Simon Laplace \cite{Laplace} in its famous essay \textit{A Philosophical Essay on Probabilities}. 

The belief in the deterministic behaviour of the nature lasted for the next century until Charles-Eugène Delaunay and Henry Poincaré introduced the first notion of chaos and perturbated systems. Poincaré stated that there is not an equation that governs the motion of three or more bodies. 

Real systems consists of a huge number of particles each one interacting with the other. Because of the number of colisions it is imposible to solve the equations of motion manually. 
Therefore, the interest in the study of the matter in the microscale put the N-body problem in the center of the focus. The way to deal with the N-body problem is through the numeral solution of that. And is then when the Molecular Dynamics (MD) simulations arrive to help us. 

The pioneering work of Adler and Wainwright \cite{Alder1957} in the fifties it was a first step in the development of the Molecular Dynamics simulations. %when they studied the phase transition of a system consisting on hard sphares.
They play the role of the experiments in real life. They provide us the observation which precedes the comprehension, because without comprehension science would be only documentation \cite{Rapaport}.
...
Along the derivation we only assume the markovian aproximation that simplify considerably the final equations, in which the transport kernels are included. 
\Pendiente{The second pillar consists on Molecular Dynamics (MD) simulations that we perform in order to validate the markovian aproximation and to compute the correlations of the transport kernels.}  
We perform MD simulations with the open source code LAMMPS \cite{Plimpton1995}. The idea behind the MD simulations is to 

The details of the simulations will be shown in the corresponding chapters. 


\section{Objetives reached in each publication}
This dissertation consist on six publications with a logical order. 
